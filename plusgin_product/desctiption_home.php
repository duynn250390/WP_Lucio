<?php
/*
Plugin Name: Đăng Tin Nhà Cần Bán
Author: Vnroki
...
function info_home_save($post_id){
...
add_action('save_post','info_home_save');

// Thông tin người nhập và liên hệ

function info_auth_meta_box_home(){
	$id = 'info_auth_meta_box_home';
	$title = 'Thông tin người đăng';
	$callback = 'info_auth_output_home';
	$screen = 'post_home';
	add_meta_box( $id, $title, $callback,$screen);

}
add_action('add_meta_boxes','info_auth_meta_box_home');

function info_auth_output_home($post){
	global $current_user;
...
function info_auth_save_home($post_id){
...
add_action('save_post','info_auth_save_home');
